...
namespace HCaptcha.Editor
{
    ...
    public class HCaptchaSetup
    {
        ...
        static void RunSetup()
        {
            ...
            Debug.Log($"[{PACKAGE_NAME}] === Checking AAR ===");

            bool aarFound = CheckAARExists();

            Debug.Log($"[{PACKAGE_NAME}] === Checking Gradle ===");
            bool gradleFound = CheckGradleTemplatesExist();

            Debug.Log($"[{PACKAGE_NAME}] === Results ===");
            Debug.Log($"[{PACKAGE_NAME}] AAR: {(aarFound ? "✓ FOUND" : "✗ NOT FOUND")}");
            Debug.Log($"[{PACKAGE_NAME}] Gradle: {(gradleFound ? "✓ FOUND" : "✗ NOT FOUND")}");

            if (aarFound && gradleFound)
            {
            ...
            {
                Debug.LogError($"[{PACKAGE_NAME}] Setup FAILED - files missing!");
        ...
        "Assets/Plugins/Android/unithhcapt-lib-release.aar",  
        "Assets/HCaptcha/Runtime/Plugins/Android/unithhcapt-lib.aar",
        "Assets/HCaptcha/Runtime/Plugins/Android/unithhcapt-lib-release.aar",  
        "Packages/com.gtrqa.hcaptcha/Runtime/Plugins/Android/unithhcapt-lib.aar",
        "Packages/com.gtrqa.hcaptcha/Runtime/Plugins/Android/unithhcapt-lib-release.aar" 
            };

            foreach (var path in paths)
            {
                bool exists = File.Exists(path);
                Debug.Log($"[{PACKAGE_NAME}]   {path}: {(exists ? "✓" : "✗")}");
                if (exists)
                    return true;
            }
        ...
        static bool CheckGradleTemplatesExist()
        {
            string[] settingsPaths = new string[]
            {
                ...
                "Assets/HCaptcha/Runtime/Plugins/Android/settingsTemplate.gradle",
            ...
            string[] mainPaths = new string[]
            {
                ...
                "Assets/HCaptcha/Runtime/Plugins/Android/mainTemplate.gradle",
            ...
            Debug.Log($"[{PACKAGE_NAME}]   Checking settingsTemplate.gradle:");
            ...
            Debug.Log($"[{PACKAGE_NAME}]   Checking mainTemplate.gradle:");
